/* Premium icon system: Lucide + semantic mapping + pastel app-tile treatment. */
(function () {
  'use strict';
  ...
  function iconForName(name) {
    ...
    const tone = ICON_TONE[name] || 'mint';
    icon.dataset.iconTone = tone;
    return icon;
  }
  ...
  function boot() {
    ...
    style.textContent = `
      ...
      .action-icon .ui-icon,.modal-icon .ui-icon,.status-icon .ui-icon{width:18px;height:18px;}
      ...
      .nav-link.active .ui-icon{stroke-width:2;}
      .dark-mode-toggle .ui-icon{margin:0;width:19px;height:19px;}
      .icon-tile,.stat-icon,.shortcut-icon,.activity-icon,.empty-icon{border-radius:15px;}
      .icon-tile[data-icon-tone="mint"],.stat-icon[data-icon-tone="mint"]{background:#eaf6ee;color:#4d9070}
      .icon-tile[data-icon-tone="lavender"],.stat-icon[data-icon-tone="lavender"]{background:#f0ebf7;color:#78649c}
      .icon-tile[data-icon-tone="blush"],.stat-icon[data-icon-tone="blush"]{background:#fbecef;color:#b86c7b}
      .icon-tile[data-icon-tone="butter"],.stat-icon[data-icon-tone="butter"]{background:#fff5dc;color:#b98222}
      .icon-tile[data-icon-tone="sky"],.stat-icon[data-icon-tone="sky"]{background:#eaf3f9;color:#47779d}
    ...
    loadLucide().then(() => {
      ...
      const observer = new MutationObserver((mutations) => {
        mutations.forEach((m) => m.addedNodes.forEach((node) => { if (node.nodeType === 1) replaceLegacyIcons(node); }));
      });
